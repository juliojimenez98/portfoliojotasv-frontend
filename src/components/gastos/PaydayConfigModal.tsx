...
const PAYDAY_TYPE_OPTIONS: { value: PaydayType; label: string; description: string; icon: string }[] = [
  {
    value: 'fixed_day',
...
export default function PaydayConfigModal({
  ...
  useEffect(() => {
    if (isOpen) {
      if (currentConfig) {
        ...
        setBusinessDaysBefore(currentConfig.businessDaysBefore?.toString() || '3');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: PaydayConfig = {
        ...
        ...(selectedType === 'business_days_before_end' && { businessDaysBefore: parseInt(businessDaysBefore) }),
        ...(selectedType === 'custom_text' && { customText: customText.trim() }),
        ...(accountId && { accountId }),
  ...
  const selectedOption = PAYDAY_TYPE_OPTIONS.find((o) => o.value === selectedType);

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="💳 Día de Pago de Sueldo" size="lg">
      <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {PAYDAY_TYPE_OPTIONS.map((opt) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className={`text-sm font-semibold leading-tight ${selectedType === opt.value ? 'text-primary' : 'text-foreground'}`}>
                    {opt.label}
                  </p>
                  <p className="text-[11px] text-foreground-subtle mt-0.5 leading-snug">{opt.description}</p>
                </div>
              </button>
            ))}
          </div>
        ...
        {selectedOption && (
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-success/10 border border-success/20">
            ...
            <div>
              <p className="text-xs text-foreground-subtle">Tu día de pago quedará como:</p>
              <p className="text-sm font-bold text-success">
                {selectedType === 'fixed_day' && fixedDay
                  ? `Día ${fixedDay} de cada mes`
                  : selectedType === 'business_days_before_end' && businessDaysBefore
